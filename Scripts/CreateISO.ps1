...
#NAME: New-IsoFile AUTHOR: Chris Wu
...
#detach SQLSetup ISO connected to any VM's
try{
    $VmExists=get-vm -ErrorAction SilentlyContinue 
    foreach ($Vmhv in $VmExists)
    {Set-VMDvdDrive -VMName $($Vmhv.Name) -Path ""}
}
...
Get-ChildItem $Sourcefolder | New-ISOFile -Path $isoPath

#Attach SQLSetup ISO to all VM's
try{
    $VmExists=get-vm -ErrorAction SilentlyContinue 
    foreach ($Vmhv in $VmExists)
    {Set-VMDvdDrive -VMName $($Vmhv.Name) -Path $isoPath}
}
Catch{"Check why error while attaching ISO $isoPath to $($Vmhv.Name)"}
